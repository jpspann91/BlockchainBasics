...
public class Block {
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
		...
		else
        {
            ...
            int leafSize = lstItems.size();
            int totalNodes = (2 * leafSize) - 1;
            int totalSplits = logBase2(leafSize);
            int splitCounter = 1;
            int lastElement = totalNodes - 1;
            double nodeIntervalSize = leafSize;
            int siblingStart = 0;
            int siblingEnd = leafSize - 1;

            MerkleNode[] merkleArray = new MerkleNode[totalNodes];

            for(int i = 0; i < totalNodes - 1 ; i++)
            {
                merkleArray[i] = new MerkleNode();
            }
            while(splitCounter <= totalSplits)
            {
                if (splitCounter == 1)
                {
                    for (int i = 0; i < leafSize; i++)
                    {
                        merkleArray[i].sHash = BlockchainUtil.generateHash(lstItems.get(i));
                    }
                    siblingStart += nodeIntervalSize;
                    nodeIntervalSize = nodeIntervalSize/ Math.pow(2,splitCounter);
                    siblingEnd += nodeIntervalSize;
                }
                else if (splitCounter > 1 && splitCounter != totalSplits && nodeIntervalSize != 1)
                {
                    for (int i = siblingStart, j = (siblingStart + (int)nodeIntervalSize); i < siblingEnd; i+=2, j++)
                    {
                        populateMerkleNode(merkleArray[j], merkleArray[i], merkleArray[i + 1]);
                    }
                    siblingStart += nodeIntervalSize;
                    siblingEnd += nodeIntervalSize/ Math.pow(2,splitCounter);
                    nodeIntervalSize = nodeIntervalSize / Math.pow(2,splitCounter);

                }
                else if (splitCounter == totalSplits)
                {
                    populateMerkleNode(merkleArray[lastElement], merkleArray[lastElement - 2], merkleArray[lastElement - 1]);
                }
                splitCounter++;
            }
            return merkleArray[lastElement].sHash;
        }
    }
